...
export default function UserPurchaseDetails({
  ...
  const [isViewingTrainingSessions, setIsViewingTrainingSessions] =
    useState(false);
    ...
    <>
      ...
      >
        <div className="relative flex items-center justify-end gap-2 min-w-20">
          {purchaseStatus?.activationRecords[0]?.user?.training_session_data
            ?.length > 0 && (
            <span
          ...
          >
            <EyeIconLoading
              ...
              strokeColor={
                ...
                purchaseStatus?.startedTraining_with_VR
                  ? "#0062ff"
      ...
      >
        {isEditing ? (
          ...
          <TrainingSessions
            sessions={
              purchaseStatus?.activationRecords[0]?.user
